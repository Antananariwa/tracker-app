...
type UseBackendStockResult = {
  data: { [symbol: string]: StockHistoryResponse | null } | null
  loading: boolean
  error: Error | null
}

const useBackendPortfolioAssets = (symbols: string[]): UseBackendStockResult => {
  const [data, setData] = useState<{ [symbol: string]: StockHistoryResponse | null } | null>(null)
  ...
  useEffect(() => {
    ...
    let allSymbolsData: { [symbol: string]: StockHistoryResponse | null } = {};
